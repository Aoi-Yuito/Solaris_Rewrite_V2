...
class Okay:
    def __init__(gateway, bot, guild):
        gateway.bot = bot
        self.guild = guild

    async def permissions(gateway):
        bot_user = await gateway.bot.cache.get_member(ctx.get_guild().id, 841547626772168704)
        perm = lightbulb.utils.permissions_for(bot_user)
        
        if not perm.MANAGE_ROLES:
            await trips.gateway(gateway, "Solaris no longer has the Manage Roles permission")
        elif not perm.KICK_MEMBERS:
            await trips.gateway(gateway, "Solaris no longer has the Kick Members permission")
        else:
            return True

    async def gate_message(gateway, rc_id, gm_id):
        try:   
            if (rc := gateway.bot.cache.get_guild_channel(rc_id)) is None:
                await trips.gateway(gateway, "the rules channel no longer exists, or is unable to be accessed by Solaris")
            else:
                # This is done here to ensure the correct order of operations.
                gm = await gateway.bot.cache.get_message(gm_id)

                perm = lightbulb.utils.permissions_in(
                    rc,
                    await ctx.bot.rest.fetch_member(
                        gateway.ctx.get_guild().id,
                        #ctx.get_guild().id,
                        841547626772168704
                    ),
                    True
                ...
                if not perm.MANAGE_MESSAGES:
                    await trips.gateway(
                        gateway, "Solaris does not have the Manage Messages permission in the rules channel"
                    )
                else:
                    return gm

        except hikari.NotFoundError:
            await trips.gateway(gateway, "the gate message no longer exists")

    async def blocking_role(gateway, br_id):
        bot_user = await gateway.bot.cache.get_member(ctx.get_guild().id, 841547626772168704)
        
        if (br := gateway.ctx.get_guild().get_role(br_id)) is None:
            await trips.gateway(gateway, "the blocking role no longer exists, or is unable to be accessed by Solaris")
        elif br.position >= bot_user.get_top_role.position:
            await trips.gateway(
                gateway, "the blocking role is equal to or higher than Solaris' top role in the role hierarchy"
            )
        else:
            return br

    async def member_roles(gateway, mr_ids):
        if mr_ids is not None:
            bot_user = await gateway.bot.cache.get_member(ctx.get_guild().id, 841547626772168704)
            
            for r in (mrs := [gateway.ctx.get_guild().get_role(int(id_)) for id_ in mr_ids.split(",")]) :
                if r is None:
                    await trips.gateway(
                        gateway, "one or more member roles no longer exist, or are unable to be accessed by Solaris"
                    )
                    return
    ...
    async def exception_roles(gateway, er_ids):
        if er_ids is not None:
            for r in (ers := [gateway.ctx.get_guild().get_role(int(id_)) for id_ in er_ids.split(",")]) :
                if r is None:
                    await trips.gateway(
                        gateway, "one or more exception roles no longer exist, or are unable to be accessed by Solaris"
                    )
                    return

            return ers

    async def welcome_channel(gateway, wc_id):
        if wc_id is not None:
            if (wc := gateway.bot.cache.get_guild_channel(wc_id)) is None:
                await trips.gateway(
                    gateway, "the welcome channel no longer exists or is unable to be accessed by Solaris"
                )
            perm = lightbulb.utils.permissions_in(
                wc,
                await ctx.bot.rest.fetch_member(
            ...
            if not perm.SEND_MESSAGES:
                await trips.gateway(gateway, "Solaris does not have the Send Messages permission in the welcome channel")
            else:
                return wc

    async def goodbye_channel(gateway, gc_id):
        if gc_id is not None:
            if (gc := gateway.bot.cache.get_guild_channel(gc_id)) is None:
                await trips.gateway(
                    gateway, "the goodbye channel no longer exists or is unable to be accessed by Solaris"
                )
            ...
            #elif not perm.SEND_MESSAGES:
            if not perm.SEND_MESSAGES:
                await trips.gateway(gateway, "Solaris does not have the Send Messages permission in the goodbye channel")
            else:
                return gc


class Synchronise:
    def __init__(gateway, bot):
        gateway.bot = bot

    async def _allow(gateway, okay, member, br_id, mr_ids):
    ...
    async def _deny(gateway, okay, member, br_id):
        await member.kick(reason="Member declined the server rules (performed during synchronisation).")

    async def members(gateway, guild, okay, gm, br_id, mr_ids, er_ids, last_commit, entrants, accepted):
        def _check(m):
            return not m.is_bot and (m.joined_at > last_commit or m.id in entrants)
        ...
        await gateway.bot.db.executemany("DELETE FROM entrants WHERE GuildID = ? AND UserID = ?", set([*reacted, *left]))
        await gateway.bot.db.executemany("DELETE FROM accepted WHERE GuildID = ? AND UserID = ?", set(left))
        await gateway.bot.db.executemany("INSERT OR IGNORE INTO accepted VALUES (?, ?)", set(new))

    async def roles(gateway, guild, okay, br_id, mr_ids, accepted, accepted_only):
        def _check(m):
        ...
        for member in filter(lambda m: _check(m), guild.get_members):
    ...
    async def reactions(gateway, guild, gm, accepted):
        tick = gm.reactions[0]
    ...
    async def on_boot(gateway):
        last_commit = chron.from_iso(await gateway.bot.db.field("SELECT Value FROM bot WHERE Key = 'last commit'"))
        records = await gateway.bot.db.records(
        ...
        entrants = {
            ...
            for guild_id, user_ids in await gateway.bot.db.records(
        ...
        for guild_id, rc_id, gm_id, br_id, mr_ids, er_ids in records:
            guild = gateway.bot.get_guild(guild_id)
            okay = Okay(gateway.bot, guild)
        ...
        await gateway.bot.db.execute("UPDATE entrants SET Timeout = datetime('now', '+3600 seconds')")
...
async def on_started(event: hikari.StartedEvent):
    ...
    gateway.bot.scheduler.add_job(remove_on_timeout(gateway), CronTrigger(second=0))
...
async def allow_on_accept(gateway, member, okay, br_id, mr_ids, wc_id, wt):
    if (br := await okay.blocking_role(br_id)) in member.roles:
...
async def remove_on_decline(gateway, member, okay, br_id):
    if (br := await okay.blocking_role(br_id)) in member.roles:
        await member.kick(reason="Member declined the server rules.")

async def allow_on_exception(gateway, member, okay, br_id, mr_ids):
    if (br := await okay.blocking_role(br_id)) in member.roles:
        if (mrs := await okay.member_roles(mr_ids)) and (unassigned := set(mrs) - set(member.roles)):
...
@lightbulb.command(name="synchronise", aliases=["synchronize", "sync"], description="Synchronise the gateway module. In theory, you should only ever need this as a convenience utility, but it is useful to use if Solaris falls out of sync for whatever reason. Use the command for information on available subcommands.",)
...
async def synchronise_members_command(ctx: lightbulb.context.base.Context) -> None:
    async with ctx.get_channel().trigger_typing():
        ...
        entrants = await ctx.bot.db.column("SELECT UserID FROM entrants WHERE GuildID = ?", ctx.get_guild().id)
        accepted = await ctx.bot.db.column("SELECT UserID FROM accepted WHERE GuildID = ?", ctx.get_guild().id)

        if gm := await okay.gate_message(rc_id, gm_id):
            await Synchronise(ctx.bot).members(
...
async def synchronise_roles_command(ctx: lightbulb.context.base.Context) -> None:
    async with ctx.get_channel().trigger_typing():
        ...
        br_id, mr_ids = (
            await ctx.bot.db.record(
                "SELECT BlockingRoleID, MemberRoleIDs FROM gateway WHERE GuildID = ?", ctx.get_guild().id,
            )
            or [None] * 2
        )
        accepted = await ctx.bot.db.column("SELECT UserID FROM accepted WHERE GuildID = ?", ctx.get_guild().id)

        await Synchronise(ctx.bot).roles(ctx.get_guild(), okay, br_id, mr_ids, accepted, accepted_only)
...
async def synchronise_reactions_command(ctx: lightbulb.context.base.Context) -> None:
    async with ctx.get_channel().trigger_typing():
        ...
        rc_id, gm_id = (
            await ctx.bot.db.record(
                "SELECT RulesChannelID, GateMessageID FROM gateway WHERE GuildID = ?", ctx.get_guild().id,
            )
            or [None] * 2
        )
        accepted = await self.bot.db.column("SELECT UserID FROM accepted WHERE GuildID = ?", ctx.get_guild().id)

        if gm := await okay.gate_message(rc_id, gm_id):
            await Synchronise(ctx.bot).reactions(ctx.guild, gm, accepted)
            await ctx.respond(f"{self.bot.tick} Gate message reactions synchronised.")
...
async def synchronise_everything_command(ctx: lightbulb.context.base.Context) -> None:
    async with ctx.get_channel().trigger_typing():
        okay = Okay(gateway.bot, ctx.get_guild())
        rc_id, gm_id, br_id, mr_ids, er_ids = (
            await ctx.bot.db.record(
                "SELECT RulesChannelID, GateMessageID, BlockingRoleID, MemberRoleIDs, ExceptionRoleIDs FROM gateway WHERE GuildID = ?",
                ctx.get_guild().id,
        ...
        if gm := await okay.gate_message(rc_id, gm_id):
            sync = Synchronise(ctx.bot)
            await sync.members(ctx.get_guild(), okay, gm, br_id, mr_ids, er_ids, last_commit, entrants, accepted)
            await sync.roles(ctx.get_guild(), okay, br_id, mr_ids, accepted, roles_for_accepted_only)
...
async def checkaccepted_command(ctx: lightbulb.context.base.Context) -> None:
    if ctx.options.target is not None:
        if await ctx.bot.db.field(
            "SELECT UserID FROM accepted WHERE GuildID = ? AND UserID = ?", ctx.options.target.guild_id, ctx.options.target.id
        ):
            await ctx.respond(f"{ctx.bot.tick} {ctx.options.target.display_name} has accepted the server rules.")
        else:
            await ctx.respond(f"{ctx.bot.cross} {ctx.options.target.display_name} has not accepted the server rules.")
    else:
        accepted = await ctx.bot.db.column("SELECT UserID FROM accepted WHERE GuildID = ?", ctx.get_guild().id)
        await ctx.respond(
            f"{ctx.bot.info} {len(accepted):,} / {len([m for m in ctx.get_guild().get_members() if not m.is_bot]):,} members have accepted the server rules."
        )
...
async def resetaccepted_command(ctx: lightbulb.context.base.Context) -> None:
    await ctx.bot.db.execute("DELETE FROM accepted WHERE GuildID = ?", ctx.get_guild().id)
